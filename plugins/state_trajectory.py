...
def PlotStateTrajectory(m):
    ...
    if m:
        ...
        if hasattr(m, 'state_trajectory'):
            ...
            ### adapted to PyPDEVS
            times_lst = map(lambda a : a[0] if isinstance(a, tuple) else a, st.keys())
...
def add_state_trajectory_menu(*args, **kwargs):
    ...
    states.SetBitmap(wx.Image(os.path.join(ICON_PATH_16_16, 'graph.png'), wx.BITMAP_TYPE_PNG).ConvertToBitmap())
