...
import Core.Utilities.NetManager as NetManager
...
def MsgBoxError(event, parent, msg):
	...
	if isinstance(msg, unicode):
		dial = wx.MessageDialog(parent, _('Error trying to import module : %s') % msg, _('Error'),
								wx.OK | wx.ICON_ERROR)
		dial.ShowModal()
	### si erreur dans le constructeur (__init__) ou pendant la simulation du .py
		...
		if path is not None:
			...
			if dial.ShowModal() == wx.ID_YES:
				...
				editor_frame = DEVSComponent.OnEditor(devscomp, event)
			...
			wx.MessageBox(
				_("There is errors in python file.\nError trying to translate error informations: %s %s %s") % (
				typ, val, tb), _("Error"), wx.OK | wx.ICON_ERROR)
...
def GetArgs(cls=None):
	...
	if inspect.isclass(cls):
		...
		return dict(zip(constructor[0][1:], constructor[3])) if constructor[3] is not None else {}
	else:
...
class AMDComponent(GenericComponent):
	...
	def BlockModelAdapter(cls, label="", specific_behavior=""):
		...
		match = [re.match('[-_a-zA-z]*collector[-_a-zA-z]*', s, re.IGNORECASE) for s in
				 clsmbr.keys() + [specific_behavior]]
...
class DEVSComponent:
	...
	def updateDEVSPriorityList(self):
		...
		assert (isinstance(self, (Container.ContainerBlock, Container.Diagram)))
		...
		if coupled_devs is not None and self.priority_list != []:
			...
			block_list = filter(lambda c: isinstance(c, Container.Block), shape_list)
			...
			# si l'utilisateur n'a pas definit d'ordre de priorite pour l'activation des modeles, on la construit
