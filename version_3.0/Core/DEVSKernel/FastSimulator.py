...
import Core.Patterns.Strategy as Strategy
import Core.Utilities.pluginmanager as pluginmanager
### avec ce flag, on gere a totalite des messages sur les ports une seul fois dans delta_ext.
WITHOUT_DELTA_EXT_FOR_ALL_PORT = True
### avec ce flag on peut faire de l'execution en paralle de modele qui s'active en meme emps mais pas avec des modeles couple dans des modele couple
...
class Sender:
	...
	#def send(self, d, msg):
		...
		#"""
		#if isinstance(d, DEVS.CoupledDEVS):
			#if msg[0] != 0 and d.blockModel.label == "Coupled_Name":
				##print dir(d)
				#print "d"
				#queue = multiprocessing.Queue() #create a queue object
				#p = multiprocessing.Process(target = worker, args = (d,msg,queue))
				#p.start()
				#print 'ee'
				#r, d.myInput, d.myOutput, d.myTimeAdvance, d.timeLast = queue.get()
				#p.join()
				#print d.myInput, d.myOutput, d.myTimeAdvance, d.timeLast
				#return r
			#else:
				#CS = CoupledSolver()
				#r = CS.receive(d, msg)
...
class CoupledSolver(Sender):
	...
	def receive(self, cDEVS, msg):
		...
		# input dictionnary to the DEVS:
			...
			for t in array.array('d', map(lambda d: d.myTimeAdvance, cDEVS.componentSet)):
...
class Simulator(Sender):
	...
	def __init__(self, model=None):
		...
		self.__algorithm = Strategy.SimStrategy1(self)
