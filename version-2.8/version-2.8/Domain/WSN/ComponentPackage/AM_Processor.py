...
class Processor (AtomicDEVS):
 ...
 def extTransition(self):
	...
	if (self.msg != None) & (self.PHASE == "FREE"):# provenant de COM
		if (self.msg.typ == "router"):
			...
			self.Activity += 1
		elif (self.msg.typ == "Update"): ## un message arrive de COM pour une mise à jour
			self.changeState("BUSY",11, 0)
			self.Activity += 1
		elif (self.msg.typ == "WhiteFlag") :  ## un message arrive de COM pour découverte du réseau
			self.changeState("BUSY",12, 0)
			self.Activity += 1
		elif (self.msg.typ == "ACK"): ## un message arrive de COM pour découverte du réseau
			self.changeState("BUSY",13, 0)
			self.Activity += 1
		elif (self.msg.typ == "OK"): ## un message arrive de COM pour découverte du réseau
			self.changeState("BUSY",14, 0)
			self.Activity += 1
		elif (self.msg.typ == ""): ## un message arrive de COM pour découverte du réseau
		...
		self.changeState("DEAD",11, 0)	 # un message arrive de Battery  on l'oriente vers COM
		 ...
		 self.changeState ("BUSY",10, 0) # ça dépend à voir
		 self.Activity += 1	 	 
 ...
 def outputFnc(self):
	if (self.PHASE == "BUSY") & (self.GO == 1) :# vers Net
		...
		self.msg.conso = 0.22 			# determine la consommation d'energie pour chaque action réalisée
 ...
 def changeState (self, phase="FREE",go = 0, sigma=__INFINITY__):
	...
	self.SIGMA = sigma
 def __str__(self):
  	return (self.myName,"Process", self.PHASE, self.GO, self.Activity)
