...
class TestMPI(unittest.TestCase):
    ...
    def test_MPI_checkpoint(self):
        ...
        proc = subprocess.Popen("mpirun -np 3 python regression/experiment.py checkpoint", shell=True)
...
def runMPI(name, processes = 3):
    ...
    try:
        #proc = subprocess.Popen("mpirun -np " + str(processes) + " python mpirunner.py regression." + str(name), shell=True)
        proc = subprocess.Popen("mpirun -np " + str(processes) + " python regression/experiment.py " + str(name), shell=True)
        proc.wait()
    except:
        import sys
