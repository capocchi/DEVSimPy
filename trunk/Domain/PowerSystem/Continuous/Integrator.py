...
## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
# Integrator.py --- Atomic Model for integration function
#                     --------------------------------
#                        	 Copyright (c) 2010
#                       	  Laurent CAPOCCHI
...
from math import fabs, sqrt, floor, acos, cos
...
class Integrator(DomainBehavior):
	...
	###
	def __init__(self, m='QSS2', dQ=0.01, x=0.0):
		"""	Constructor.
		"""
	...
	def extTransition(self):
		...
		if(self.m == "QSS"):
			...
			c=0.0
			s=0.0
			self.x+=self.u*self.elapsed+self.mu/2*self.elapsed*self.elapsed
			self.u=Aux[0]
			self.mu=Aux[1]
			...
			self.x+=self.u*self.elapsed+(self.mu*self.elapsed*self.elapsed)/2+(self.pu*self.elapsed*self.elapsed*self.elapsed)/3
			self.u=Aux[0]
			self.mu=Aux[1]
			self.pu=Aux[2]
			
			if(self.state['sigma'] != 0):
				a=0.0;b=0.0;c=0.0; v=0.0;w=0.0;A=0.0;B=0.0;i1=0.0;i2=0.0;s=0.0

				self.q+=self.mq*self.elapsed+self.pq*self.elapsed*self.elapsed
	...
	###
  	def outputFnc(self):
		...
		if self.timeNext == 0:
			...
			if(self.m=="QSS"):
				if(self.u==0):
					self.msg.value=[self.q,0,0]
				else:
					self.msg.value=[self.q+self.dQ*self.u/fabs(self.u), 0, 0]
				self.msg.time=self.timeNext
				self.poke(self.OPorts[0], self.msg)
						
			elif(self.m=="QSS2"):
				assert( self.msg != None)
				self.msg.value=[self.x+self.u*self.state['sigma']+self.mu*self.state['sigma']*self.state['sigma']/2, self.u+self.mu*self.state['sigma'], 0]
				self.msg.time=self.timeNext				
				self.poke(self.OPorts[0], self.msg)
				
			else:
				assert( self.msg != None)
				self.msg.value=[self.x+self.u*self.state['sigma']+(self.mu*pow(self.state['sigma'],2))/2.0 + (self.pu*pow(self.state['sigma'],3))/3.0, self.u+self.mu*self.state['sigma']+self.pu*pow(self.state['sigma'],2), self.mu/2.0 + self.pu*self.state['sigma']]
				self.msg.time=self.timeNext		
				self.poke(self.OPorts[0], self.msg)
