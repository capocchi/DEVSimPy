...
from Core.DEVSKernel.DEVS import CoupledDEVS
...
class Sender(Subject):
	...
	def send(self, d, msg):
		...
		if isinstance(d, CoupledDEVS):
			...
			pluginmanager.trigger_event("SIM_BLINK", model=d, msg=msg, sender=self)
...
class CoupledSolver(Sender):
	...
	def parallel_send(self, Y, cDEVS, t):
	...
	def receive(self, cDEVS, msg):
		...
		if msg[0] == 1:
			...
			self.parallel_send(send(dStar, msg), cDEVS, t)
			...
			#mgr = Manager()
			#d = mgr.list()
			#jobs = [Process(target = self.parallel_send, args = (d, send(dStar, msg), cDEVS, t)) for dStar in cDEVS.immChildren]
			#for j in jobs:
			#j.start()
			#for j in jobs:
			#j.join()
			...
			#cDEVS.myOutput = {}
			#for c in d:
			#cDEVS.myOutput.update(c[1])

			#cDEVS.myTimeAdvance = min([c[0] for c in d]+[cDEVS.myTimeAdvance])

			#cDEVS.immChildren = [d for d in cDEVS.componentSet if cDEVS.myTimeAdvance == d.myTimeAdvance]

			###
			#while (cDEVS.immChildren != []):

			#dStar = cDEVS.select(cDEVS.immChildren)
			#index = cDEVS.immChildren.index(dStar)
		...
		# input dictionnary to the DEVS:
			...
			self.parallel_send(cDEVS.myInput, cDEVS, t)
