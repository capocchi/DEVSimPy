...
#                     --------------------------------
# Version 3.0                                        last modified: 13/05/10
## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
#
...
class RecentFileMenu(wx.Menu):
	...
	def __init__(self, parent):
		...
		# affichage du menu des dernier fichier consulte avec gestion des fichier qui n'existe plus 
...
class DiagramMenu(wx.Menu):
	...
	def __init__(self, parent):
		...
		infoDiagram = wx.MenuItem(self, ID_INFO_DIAGRAM, _('Information'),
...
class ItemLibraryPopupMenu(wx.Menu):
	...
	def __init__(self, parent):
		...
		if not parent.ItemHasChildren(parent.GetSelection()) and hasattr(parent, 'OnItemDocumentation'):
			...
			self.Bind(wx.EVT_MENU, parent.OnItemEdit, id=ID_EDIT_LIB)    # attention a le mettre avant le popUpMenu
			self.Bind(wx.EVT_MENU, parent.OnItemRename, id=ID_RENAME_LIB)    # attention a le mettre avant le popUpMenu
			self.Bind(wx.EVT_MENU, parent.OnItemDocumentation, id=doc.GetId())    # attention a le mettre avant le popUpMenu
			self.Bind(wx.EVT_MENU, parent.OnItemRefresh, id=ID_UPDATE_LIB)    # attention a le mettre avant le popUpMenu
		...
		self.Bind(wx.EVT_MENU, parent.OnDelete, id=ID_DELETE_LIB)    # attention a le mettre avant le popUpMenu
...
class ShapePopupMenu(wx.Menu):
	...
	def __init__(self, shape, event):
		...
		#self.__canvas.Bind(wx.EVT_MENU, shape.OnRename, id = ID_RENAME_SHAPE)
			...
			for i, item in enumerate(filter(lambda a: a != shape and not isinstance(a, Container.ConnectionShape),
											self.__canvas.GetDiagram().GetShapeList())):
				# on evite de proposer les connections suivante: iPort->iPort, oPort->oPort
				if (isinstance(shape, Container.iPort) and not isinstance(item, Container.iPort)) or (
					isinstance(shape, Container.oPort) and not isinstance(item, Container.oPort)) or isinstance(shape,
																												Container.Block):
