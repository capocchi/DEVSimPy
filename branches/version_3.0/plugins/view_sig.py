...
sft_lst = {'Google Earth': os.path.join(ICON_PATH_16_16, 'googleearth-icon.png'),
		   'Google Maps': os.path.join(ICON_PATH_16_16, 'firefox.png')}
...
def start_sig_viewer(*args, **kwargs):
	...
	if 'Maps' in model.software:
		...
		webbrowser.open(
			'http://maps.google.fr/maps?f = q&source = s_q&hl = fr&geocode = &q = http:%2F%2Flcapocchi.free.fr%2Fdevsimpy%2Fout.kml&sll = 46.75984,1.738281&sspn = 11.456796,25.378418&ie = UTF8&z = 10')
	...
	### if google earth has been chose
		...
		if "wxMSW" in wx.PlatformInfo:
			...
			try:
				os.system(which.which('googleearth') + ' &')
			except:
...
def Config(parent):
	...
	frame = ConfigFrame(parent, wx.ID_ANY, title=_('SIG Viewer'), size=(700, 450),
...
class ConfigFrame(Extended):
	...
	def __init__(self, parent, id=wx.ID_ANY, title="", pos=wx.DefaultPosition, size=wx.DefaultSize,
		...
		self.statusbar = FTPGUI.FTPStatusBar(self)
	...
	def CreateRemainingSpace(self):
		...
		self.cb.SetChecked([index for index in range(self.cb.GetCount()) if
							self.diagram.GetShapeByLabel(self.cb.GetString(index)).__class__ == SIGViewer])
	...
	def ReCreateFoldPanel(self, fpb_flags):
		...
		if self.cb.GetCount() != 0:
			...
			self._pnl.AddFoldPanelWindow(item, wx.StaticText(item, wx.ID_ANY, _('Ftp site'), (20, 20)),
										 fpb.FPB_ALIGN_WIDTH, 5, 20)
			self._pnl.AddFoldPanelWindow(item, self.ftpsite, fpb.FPB_ALIGN_WIDTH, 5, 20)

			self._pnl.AddFoldPanelWindow(item, wx.StaticText(item, wx.ID_ANY, _('Ftp directory'), (20, 60)),
										 fpb.FPB_ALIGN_WIDTH, 5, 20)
			self._pnl.AddFoldPanelWindow(item, self.ftpdir, fpb.FPB_ALIGN_WIDTH, 5, 20)

			self._pnl.AddFoldPanelWindow(item, wx.StaticText(item, wx.ID_ANY, _('Login'), (20, 100)),
										 fpb.FPB_ALIGN_WIDTH, 5, 20)
			self._pnl.AddFoldPanelWindow(item, self.login, fpb.FPB_ALIGN_WIDTH, 5, 20)

			self._pnl.AddFoldPanelWindow(item, wx.StaticText(item, wx.ID_ANY, _('Password'), (20, 150)),
										 fpb.FPB_ALIGN_WIDTH, 5, 20)
	...
	def OnOk(self, evt):
		...
		for index in range(self.cb.GetCount()):
			...
			shape.__class__ = SIGViewer if self.cb.IsChecked(index) else Container.CodeBlock
...
#--------------------------------------------------
class SIGViewer(Container.CodeBlock):
	""" SIGViewer(label)
	"""
