...
def Config(parent):
	...
	frame = wx.Frame(parent, wx.ID_ANY, title=_('Myth Music Viewer'),
	...
	num = cb.GetCount()
	cb.SetChecked(
		[index for index in range(num) if diagram.GetShapeByLabel(cb.GetString(index)).__class__ == MythMusicViewer])
	...
	def OnOk(evt):
		...
		for index in range(cb.GetCount()):
			...
			shape.__class__ = MythMusicViewer if cb.IsChecked(index) else Container.CodeBlock
...
#--------------------------------------------------
class MythMusicViewer(Container.CodeBlock):
