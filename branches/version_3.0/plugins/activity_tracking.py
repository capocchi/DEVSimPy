...
try:
	...
	sys.stdout.write(
		'Install psutil module for better cpu result of the plugin.\n The time module has been imported pending.')
		...
		if self._master is not None:
			...
			for m in GetFlatDEVSList(self._master, []):
				if hasattr(m, 'texec'):
					...
					source_list = zip(['extTransition', 'intTransition', 'outputFnc', 'timeAdvance'],
									  map(inspect.getsource,
										  [m.__class__.extTransition, m.__class__.intTransition, m.__class__.outputFnc,
										   m.__class__.timeAdvance]))
					...
					for fct, text in source_list:
						...
						for graph in visitor.graphs.values():
							...
							if WRITE_DOT_TMP_FILE and fct in 'extTransition':
								dot_path = os.path.join(tempfile.gettempdir(),
														"%s(%s)_%s.dot" % (label, str(m.myID), fct))
			...
			data = map(lambda a, i, b, c, d, e: (a, i, b / H, d / H, c, e), model_name_list, model_id_list,
					   quantitative_activity_list, cpu_activity_list, weighted_activity_list, mcCabe_activity_list)
	...
	if master is not None:
		...
		for m in GetFlatDEVSList(master, []):
			...
			if hasattr(m, 'texec') and selected_label == label:
				...
				PlotGUI.PlotManager(canvas, _("CPU Activity"), m, xl="Time [s]", yl="CPU time")

	else:
		dial = wx.MessageDialog(event.GetEventObject(), _(
			'Master DEVS Model is None ! \nGo ti the simulation process in order to perform activity tracking.'),
...
def Config(parent):
	...
	frame = wx.Frame(parent, wx.ID_ANY, title=_('Activity Tracking'),
					 style=wx.DEFAULT_FRAME_STYLE | wx.CLIP_CHILDREN | wx.STAY_ON_TOP)
	panel = wx.Panel(frame, wx.ID_ANY)

	#lst_1  = map(lambda a: a.label, filter(lambda s: isinstance(s, Container.CodeBlock), diagram.GetShapeList()))
	...
	if L1:
		cb1.SetChecked(L1)
		### tout les block on la meme liste de function active pour le trace, donc on prend la premiere
	...
	def OnOk(evt):
		...
		for index in range(num1):
			...
			assert (isinstance(shape, Container.Block))
...
def UnConfig():
	...
	lst = map(lambda a: a.label, filter(lambda s: isinstance(s, Container.CodeBlock), diagram.GetShapeList()))
