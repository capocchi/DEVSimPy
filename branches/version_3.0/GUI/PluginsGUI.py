...
class BlockPluginsList(CheckListCtrl, Populable):
	...
	def Populate(self, model):
		...
		if not self.is_populate:
			...
			if not hasattr(self.model, "plugins"):
...
class ModelPluginsManager(wx.Frame):
	...
	def OnAdd(self, event):
		...
		dir = HOME_PATH
		open_dlg = wx.FileDialog(self, message=_('Choose a file'), defaultDir=dir, defaultFile='plugins.py',
	...
	def OnDelete(self, event):
		...
		dial = wx.MessageDialog(self, _('Do You realy want to delete plugins file?'), 'Question',
