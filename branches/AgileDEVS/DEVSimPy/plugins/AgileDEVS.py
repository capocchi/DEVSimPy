...
def retrieve_no_gen_models(master):
	global no_gen_models
	for v in master.IC:
		no_gen = v[1][0]
		if not no_gen in no_gen_models:
			no_gen_models.append(no_gen)
...
def start_test(**kwargs):
	...
	### Add control test model for dynamic injection into models inputs-------
	# Create a Mock object with AtomicDEVS spec
	control_model = Mock()
	control_model.addOutPort()
	control = control_model
	# control_model = Mock(spec=AtomicDEVS)

	master = diagram.getDEVSModel()
	retrieve_no_gen_models(master)
	master.addSubModel(control_model)
	frame.AddControlModel(control_model)
	diagram.priority_list.insert(0, "Mock")
	### ----------------------------------------------------------------------
...
def test_manager(**kwargs):
	...
	if model.state['status'] == "GENERATOR":
		print "=============================================================================="
		print "=====================================GENERATOR================================"
		print "=============================================================================="
	...
	if hasattr(model, 'getBlockModel'):
		...
		if previous:
			master.disconnectPorts(control.OPorts[0], previous.IPorts[0])

		if model and model in no_gen_models:
			master.connectPorts(control.OPorts[0], model.IPorts[0])

		print "\n\n--------------------------------------------------------------------------\n\n"
		print "Next model: %s"%block.label
		if previous and hasattr(previous, "blockModel"):
			print "Actual model: %s"%previous.blockModel.label
		print "\n\n--------------------------------------------------------------------------"
		print "\n-----------------------------------IC-------------------------------------\n\n"
		for k in master.IC:
			print k
		...
		frame.UpdateCurrentBlock(block)
		frame.UpdateCurrentMessage(msg[0])
		...
		if canvas is not None and isinstance(frame, wx.Frame):
			...
			### ta checking
			if msg[0] is 0:
				frame.UpdateCurrentColor(wx.GREEN)
			### internal transition
			elif msg[0] is 1:
				frame.UpdateCurrentColor(wx.BLUE)
			### external transition
			elif type(msg[0]) == type({}):
				frame.UpdateCurrentColor(wx.RED)

			else:
				frame.UpdateCurrentColor(wx.BLACK)
...
class TestFrame(wx.Frame):
	...
	# NOTE: TestFrame :: AddControlModel	=> note
	def AddControlModel(self, model):
		self.control_model = model

	# NOTE: TestFrame :: UpdateCurrentBlock	=> note
	def UpdateCurrentBlock(self, block):
		self.currentBlock = block

	# NOTE: TestFrame :: UpdateCurrentColor	=> note
	def UpdateCurrentColor(self, color):
		self.currentColor = color

	# NOTE: TestFrame :: UpdateCurrentMessage	=> note
	def UpdateCurrentMessage(self, message):
	...
	# NOTE: TestFrame :: GetCurrentModel	=> note
	...
	def OnForward(self, event):
		...
		log = ""
		ta = "\n%s ta checking\n" % self.currentBlock.label
		deltInt = "\n%s internal transition\n" % self.currentBlock.label
		deltExt = "\n%s external transition\n" % self.currentBlock.label

		### Log frame update
		if self.currentColor is wx.GREEN:
			log = ta
		elif self.currentColor is wx.BLUE:
			log = deltInt
		elif self.currentColor is wx.RED:
			log = deltExt

		self.logFrame.SetDefaultStyle(wx.TextAttr(self.currentColor))
...
### -----------------------------------------------------------------------------------------------
# NOTE: Mock :: Mocking model
class Mock(DomainBehavior):
	"""docstring for Mock"""

	def __init__(self):
		DomainBehavior.__init__(self)
		self.state = { 'status': 'GENERATOR', 'sigma': 0}
		self.msg = Message()
