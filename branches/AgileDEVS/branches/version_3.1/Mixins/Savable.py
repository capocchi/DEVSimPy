...
import Core.Utilities.XML as XML
...
class DumpBase(object):
	...
	### Recupere les fils de la classe passe en parametre, ici on veut que ce soit DumpBase, on les ajoute a une liste (subclasses),
	### ensuite pour chaque element de la liste on recupere la variable static ext (list),
	### et on ajoute au dictionnaire DumpBase.vars en cle l'extension ext et en valeur le nom de la classe correspondante
	@staticmethod
	def PopulateDB():
		"""
		"""
...
class DumpZipFile(DumpBase):
	...
	def Load(self, obj_loaded, fileName=None):
		...
		#obj_loaded.python_path = os.path.join(obj_loaded.model_path, re.findall("([\w]*[%s])*([\w]*.py)"%os.sep, obj_loaded.python_path)[0][-1])
		# if not os.path.exists(os.path.dirname(obj_loaded.python_path)):
			# sepUsed = ""
			# sepLinux = "/"
			# sepWindows = "\\"
			# if obj_loaded.python_path.count(sepWindows):
				# sepUsed = sepWindows
			# else:
				# sepUsed = sepLinux
			# print sepUsed
			# print re.findall(".*.[a|c]md\%s([^\%s]*.py)"%(sepUsed, sepUsed), obj_loaded.python_path)
			# obj_loaded.python_path = os.path.join(obj_loaded.model_path, re.findall(".*.[a|c]md\%s([^\%s]*.py)"%(sepUsed, sepUsed), obj_loaded.python_path)[0])
...
class DumpGZipFile(DumpBase):
	...
	def Load(self, obj_loaded, fileName=None):
		...
		try:
			...
			try:
				dsp = cPickle.load(f) #here, l'erreur est ici
			except Exception, info:
				sys.stderr.write(_("Problem loading: %s -- %s\n") % (str(fileName), info))
				return info
			finally:
...
class DumpXMLFile(DumpBase):
	...
	def Save(self, obj_dumped, fileName=None):
		...
		XML.makeDEVSXML(label, D, fileName)
