...
class FileGenerator(Generator):
	...
	def __init__(self, fileName=os.path.join(os.getcwd(), "fichier.csv"), listValues=None, time=0, outPutFrequency=1.0,
				 comma=" "):
		"""
		...
		"""
		if not listValues: listValues = [1]
		...
		if self.file_error_flag:
			with open(fileName, "rb") as f:
				for i,l in enumerate(filter(lambda b: b != '' , map(lambda a: a.replace('\n', ''), f.readlines()))):
					...
					row = filter(lambda a: a!='', l.split(self.__comma))
					lenght = len(row)
						
					### listValues est vide, liste de temps remplit avec entiers dependant de la fréquence de sortie
					if self.list_empty_flag :
						t = i*self.outPutFrequency
					### listValues non vide alors la colonne de temps est spécifier par time
					else:
						t = row[self.time]
					...
					if self.list_empty_flag:
						...
						for val in self.V.keys():
							### si l'utilsiateur demande une colonne superieur au nombre de colonne dans le fichier -> derniere selectionnée
							if val >= lenght:
								self.V[val].append(row[lenght-1])
							### l'utilisateur demande une colonne inferieur a 0 ou egale a 0 (colonne de temps) -> premiere selectionnée
							elif val < 0 or val == 0:
								self.V[val].append(row[1])
							else:
								self.V[val].append(row[val])
