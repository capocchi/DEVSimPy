...
import Core.Components.Decorators as Decorators # redirectStdout
import Core.Utilities.ReloadModule as ReloadModule
import Core.Utilities.Utilities as Utilities # path_to_module
...
class EditionNotebook(wx.Notebook):
	...
	def AddEditPage(self, title, path):
		...
		#Todo :: Regler le probleme d'edition
		L = re.findall("(.*\.[\w]+)\%s(.*)" % os.sep, path) #here
		#G = re.findall("(.*\.(amd|cmd))\%s(.*)" % os.sep, path)

		fileCode = ""

		if L:
			model_path, name = L.pop(0)
	...
	def OnReIndent(self, event):
		...
		### reindent from file
		os.system("python %s" % os.path.join(HOME_PATH, "Core", "Utilities", "reindent.py") + " " + python_file)
...
class BlockEditor(Editor):
	...
	def UpdateModule(self):
		...
		### re importation du module de la classe avec verification des erreurs eventuelles dans le code
		if hasattr(self.cb, 'model_path') and zipfile.is_zipfile(self.cb.model_path):
			module_name = self.cb.model_path

		else:
			# recuperation du module correspondant a� la classe
			module_name = Utilities.path_to_module(self.cb.python_path)
