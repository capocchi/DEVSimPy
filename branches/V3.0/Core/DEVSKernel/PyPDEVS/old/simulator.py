...
class Simulator(object):
    ...
    def loadCheckpoint(self, GVT):
        ...
        locations = range(COMM_WORLD.Get_size())
        for i in locations:
            # Construct the ring
    ...
    ###
    def simulate(self, termination_condition=None, termination_time=float('inf'), verbose=False, xml=False, vcd=False,
                 termination_location=None, logaddress=('localhost', 514), loglevel=logging.WARN, outfile=None,
                 GVT_freq=1, CHK_freq=-1, state_saving=2, seed=1, allowNested=True, manualCopy=False,
                 disallowIrreversible=False, realTimeInputPortReferences=None, realtime=False, generatorfile=None,
                 subsystem="python"):
        #TODO update
        """
        ...
        """
        if not realTimeInputPortReferences: realTimeInputPortReferences = {}
        ...
        try:
            ...
            if middleware.USE_MPI:
                loclist = range(COMM_WORLD.Get_size())
            else:
    ...
    def real_simulate(self, locations = range(COMM_WORLD.Get_size())):
        try:
