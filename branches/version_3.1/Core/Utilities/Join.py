...
def makeJoin(diagram=None, addInner=None, liaison=None, model=None, bool=False, x=None, y=None, labelEnCours=""):
	"""
	"""
	...
	if not x: x = [40]
	if not model: model = {}
	if not liaison: liaison = []
	if not addInner: addInner = []
	# print "--------------------------------------"
...
def makeAddInner(c, addInner):
	import Core.Components.Container as Container
	# Re-verification pour ere sur qu'il s'agit bien d'un modele couple
...
def coupledStruct(c):
	...
	shapeList = c.GetShapeList()
	for s in shapeList:
	#On met le boolean a true, pour signaler un modele couple dans le tableau des enfants.
