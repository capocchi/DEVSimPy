...
#                     --------------------------------
# Version 3.1                                        last modified: 04/03/2013
## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
#
...
class ShapeCanvas(wx.ScrolledWindow, Observer.Subject):
	...
	def OnDelete(self, event):
		...
		for s in self.select():
   			name = _("Connexion") if isinstance(s, Container.ConnectionShape) else s.label
	...
	def DiagramModified(self):
		...
		if self.diagram.modify:
			...
			for string, win in D.items():
				Components.printOnStatusBar(win.statusbar, {0: "%s %s" % (string, _("modified")), 1: os.path.basename(diagram.last_name_saved), 2: ''})
