...
### --------------------------------------------------------------------------
class GeneralNotebook(Observer.Observer):
	def __init__(self, *args, **kwargs):
		...
		libPanel = LibPanel.LibPanel(self, self.labelList[0])
		propPanel = PropPanel.PropPanel(self, self.labelList[1])
			...
			if propPanel:
				...
				if model:
					if model != self.selected_model:
						newContent = AttributeEditor.AttributeEditor(propPanel, wx.ID_ANY, model, canvas)
