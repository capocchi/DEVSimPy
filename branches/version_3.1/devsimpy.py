...
builtin_dict = {'SPLASH_PNG': os.path.join(ABS_HOME_PATH, 'Assets', 'bitmaps', 'splash.png'),
				...
				'DEVS_DIR_PATH_DICT':{'PyDEVS':os.path.join(ABS_HOME_PATH,'Core','DEVSKernel','PyDEVS'),
									'PyPDEVS_221':os.path.join(ABS_HOME_PATH,'Core','DEVSKernel','PyPDEVS','pypdevs221' ,'src'),
									'PyPDEVS':os.path.join(ABS_HOME_PATH,'Core','DEVSKernel','PyPDEVS','old')}
				}
...
sys.stdout.write("%s" % __builtin__)
...
import GUI.LibPanel as LibPanel
import GUI.PropPanel as PropPanel
...
import GUI.ControlNotebook as ControlNotebook
...
class MainApplication(wx.Frame):
	...
	def __init__(self, parent, id, title):
		...
		# NoteBook
		self.nb1 = ControlNotebook.ControlNotebook(self, wx.ID_ANY, style=wx.CLIP_CHILDREN)
		self.tree = self.nb1.GetTree()
		self.searchTree = self.nb1.GetSearchTree()
		self.search = self.nb1.GetSearch()
		...
		if self.cfg.Exists('version'):
			...
			if not rewrite:
				...
				try:
					...
					wx.MessageBox('Error trying to read the builtin dictionary from config file. So, we load the default builtin', _('Configuration'), wx.OK | wx.ICON_INFORMATION)
	...
	def GetDiagramByWindow(self, window):
		...
		# la fenetre par laquelle a ete invoque l'action peut ̻tre principale (wx.App) ou detachee (DetachedFrame)			
