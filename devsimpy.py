...
def install_and_import(package):
	...
	try:
		...
		try:
			input(_("Press Enter to continue (Ctrl+C to skip)"))
		except SyntaxError:
			sys.exit()
		else:
			try:
	...
	try:
		...
		try:
			...
			if r == '1':
				...
				input("Press Enter to continue wxPython installation (Ctrl+C to skip)")
				
				### install wx package form whl file
				pip.main(['install', whl_path])
				
				input("Press Enter to to patch wxPython with the corrected plot.py file (Ctrl+C to skip)")
